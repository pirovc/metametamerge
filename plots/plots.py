...
def main():	
	...
	parser.add_argument('-n', metavar='<names_file>', type=str, dest="names_file", help="...")	
	parser.add_argument('-e', metavar='<nodes_file>', type=str, dest="nodes_file", help="...")
	...
	#import shelve
	#o = shelve.open("tax20160928.db")
	#all_names_scientific = o["all_names_scientific"]
	#all_names_other = o["all_names_other"]
	#nodes = o["nodes"]
	#o.close()
	...
	for idx,input_file in enumerate(args.input_files):
		T.append(Tools(input_file, identifiers[idx], methods[idx], parse_files(input_file, methods[idx], all_names_scientific, all_names_other, nodes, ranks), ranks))

	# Ground truth
	G = Tools(args.ground_truth, 'gt', 'p', parse_files(args.ground_truth, 'p', all_names_scientific, all_names_other, nodes, ranks), ranks)
